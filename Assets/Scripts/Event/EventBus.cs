...
public class EventBus
{
    ...
    private static void OnListenerAdding(EventTypes eventType, Delegate callBack)
    {
        ...
        {
            throw new Exception(string.Format("Type to add different event{0}，current event for {1}，add event {2}", eventType, d.GetType(), callBack.GetType()));
        }
    }
    private static void OnListenerRemoving(EventTypes eventType, Delegate callBack)
    {
        if (m_EventTable.ContainsKey(eventType))
        {
            ...
            if (d == null)
            {
                throw new Exception(string.Format("Remove listener error", eventType));
            }
